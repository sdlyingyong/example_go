...
//队列
//支持的方法 : 获取长度 是否为空 获取容量
//enqueue插入元素到末尾 dequeue从头部拿出元素 查看队首元素
//toString

//获取长度
func (mq MyQueue) GetLen() int {
	return mq.data.GetLen()
}

//是否为空
func (mq MyQueue) IsEmpty() bool {
	return mq.data.IsEmpty()
}

//获取容量
func (mq MyQueue) GetSize() int {
	return mq.data.GetSize()
}

//插入元素到队尾
func (mq MyQueue) enqueue(value int) (err error) {
	return mq.data.AddLast(value)
}

//从queue头部取出数据
func (mq MyQueue) dequeue() (value interface{}) {
	return mq.data.Remove(0)
}

//查看queue头部元素
func (mq MyQueue) peek() (value interface{}) {
	return mq.data.Get(0)
}
